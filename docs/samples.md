...
## Example 3: Barchart of the Concepts and their counts

```shmarql
# shmarql-view: barchart
# shmarql-label: Concept counts

select (count(?s) as ?value) (?Concept as ?label) where {
  [] a ?Concept .
  ?s ?pp ?Concept .
  filter( ?Concept != <https://schema.org/DefinedTerm> )
  filter( ?Concept != nfdicore:Standard )
} group by ?Concept
```
